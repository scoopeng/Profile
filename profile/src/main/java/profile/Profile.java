	...
	private static void profileFile(CompareContext cc, boolean deleted)
	{
		try
		{
			if (deleted)
			{
				...
				try
				{
					lines = Files.readAllLines(cc.originalFile.toPath(), Charset.forName("ISO-8859-1"));
				} catch (Exception e)
				{
					lines = Files.readAllLines(cc.originalFile.toPath());
				}
			...
			} else
			{
				...
				try
				{
					lines = Files.readAllLines(cc.revisedFile.toPath(), Charset.forName("ISO-8859-1"));
				} catch (Exception e)
				{
					lines = Files.readAllLines(cc.revisedFile.toPath());
				}
